...
def analyze_jobs_api():
    ...
    try:
        ...
        print(f"[OK] Found jobs.py at: {jobs_file}")
        print()

        # Check if the problematic code exists
        if 'status_filter in [\'running\', \'pending\', \'downloading\']' in content:
            print("[OK] Found correct authentication logic")
            print()

            # Check if debug logging exists
            if '[JOBS API]' in content:
                print("[OK] Debug logging already added")
                print()
            else:
                print("[WARN] Debug logging NOT found")
        ...
        print(f"[ERROR] ERROR: Could not find {jobs_file}")
        return False
...
def test_api_directly():
    ...
    try:
        ...
        if "message" in data and "Login" in data["message"]:
            print("[WARN] ISSUE CONFIRMED: API returning 'Login' message")
            print()
            ...
            print("[OK] API responding correctly!")
            print(f"  Jobs count: {len(data.get('jobs', []))}")
            if len(data.get('jobs', [])) == 0:
                print()
                print("[INFO] No jobs found (expected if no downloads started)")
                print("  Create a download job to see Active Downloads")
            print()

    except requests.exceptions.ConnectionError:
        print("[ERROR] ERROR: Could not connect to Flask server")
